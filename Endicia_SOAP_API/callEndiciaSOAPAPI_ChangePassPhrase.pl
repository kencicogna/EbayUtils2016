...
my $passPhrase='WelcomeToTheJungle'; 
my $newPassPhrase='WeGotFunAndGames'; 

#my $token = read_file('token.txt') or die;
my $call  = 'ChangePassPhrase';                                                      # Method name
...
my $elems = SOAP::Data->name("${call}Request" => \SOAP::Data->value( 
                  ...
                  SOAP::Data->name('NewPassPhrase'   => $newPassPhrase)->type('string'),
                  )
...
#print Dumper($elems); exit;

# my $response = $soap->call(
#         SOAP::Data->name($call)->attr({xmlns=>$uri}) =>
#           SOAP::Data->value( SOAP::Data->name("${call}Request")->attr({TokenRequested=>1}) =>
#                SOAP::Data->value( SOAP::Data->name('RequesterID')->value($acct)->type('string') ),
#           )
#       );
